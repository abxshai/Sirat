...
def clean_member_name(name):
    ...
    If name is empty after stripping, return '~ Unknown'."""
    cleaned = name.strip()
    if not cleaned:
        return "~ Unknown"
...
def process_chunk(chunk, patterns):
    ...
    If a match is found, check whether the message text (cleaned)
    exactly equals the sender’s name followed by " left" (case-insensitive).
    If so, record this as an exit event.
    Then process join events and any fallback left events.
    """
    messages = []
    joins = []
    exits = []
...
def create_exit_events_table(stats):
    ...
    This version lists each unique member (based on the first exit event recorded).
    """
    unique_exits = {}
    for event in stats.get('exit_events', []):
        user = event['user']
        # Only keep the first exit event per member
        if user not in unique_exits:
            unique_exits[user] = event
    if not unique_exits:
        return pd.DataFrame()
    df = pd.DataFrame(list(unique_exits.values()))
